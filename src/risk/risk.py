...
def get_orders(signals: pd.DataFrame):
    ...
    # generate new potisions given the signals generated by strategy and the available cash
    ...
    target_portfolio['value_usd'] = np.where(
        np.abs(target_portfolio['value_usd']) <= total_equity*0.05, 
        target_portfolio['value_usd'], 
        total_equity*0.5
    )
